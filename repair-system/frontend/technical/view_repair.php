...
// ตรวจสอบว่าช่างคนนี้ได้รับมอบหมายงานนี้หรือยัง
$stmt_assigned = $conn->prepare("
    SELECT COUNT(*) as count
    FROM repair_detail
    WHERE repair_id = :repair_id AND technical_id = :technical_id
");
$stmt_assigned->execute([
    ':repair_id' => $repair_id,
    ':technical_id' => $technical['id']
]);
$is_assigned = $stmt_assigned->fetch(PDO::FETCH_ASSOC)['count'] > 0;
?>

<!doctype html>
<html lang="th">
<head>
    <title>ดูรายละเอียดงานซ่อม #<?=$repair['id']?> | ช่างเทคนิค</title>
    <meta charset="utf-8">
...
<div class="container mt-4">
    ...
    <div class="card">
        <div class="card-header">
            <h4 class="mb-0">รายละเอียดงานซ่อม #<?=$repair['id']?></h4>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <form action="../../backend/repair_detail_action.php" method="post">
                        ...
                        <?php if (!$is_assigned): ?>
                            <button type="submit" name="update_status" value="รับงาน" class="btn btn-success" onclick="document.querySelector('select[name=status]').value='รอซ่อม'">รับงาน</button>
                        <?php endif; ?>
                    </form>
                </div>
            </div>
